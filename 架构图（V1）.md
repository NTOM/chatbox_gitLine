...
│   │   │   │   ├── MessageDetailPanel.tsx      # 消息详情面板
│   │   │   │   ├── TreeToolbar.tsx             # 树形图工具栏
│   │   │   │   ├── SelectionBoundingBox.tsx    # 框选工具
...
```mermaid
graph TD
    A[TanStack Router 路由层]
    
    subgraph Pages[页面组件 routes]
        B1[index.tsx 主聊天]
        B2[session 会话详情]
        B3[settings 设置]
    end
    
    subgraph Components[业务组件 components]
        C1[Message MessageList]
        C2[Sidebar SessionList]
        C3[Markdown Mermaid]
        C4[ConversationTree 节点视图]
    end
...
```mermaid
graph TD
    A[Vercel AI SDK 统一接口]
    
    subgraph Providers[模型提供商适配器]
        B1[OpenAI Provider]
...
graph TB
    ...
    subgraph StoreLayer[状态管理层]
        ...
        ViewModeStore[viewModeStore<br/>视图状态管理]
    end

    subgraph AdapterLayer[适配层]
        TreeAdapter[conversation-tree-adapter<br/>Session ↔ 树形结构转换]
        TreeLayout[tree-layout<br/>dagre 布局算法]
    end
...
```mermaid
flowchart LR
    A[Session<br/>原始数据] --> B[sessionToConversationTree<br/>构建树结构]
    B --> C[buildActivePathIds<br/>计算活跃路径]
    C --> D[processForks<br/>处理分支]
    D --> E[applyTreeLayout<br/>dagre布局]
    E --> F[ConversationTree<br/>带位置信息]
    F --> G[ReactFlow<br/>渲染]
```
...
flowchart LR
    A[用户操作] --> B{操作类型}
    B -->|拖拽节点| C[handleNodeDragStop]
    B -->|平移/缩放| D[useOnViewportChange.onEnd]
...
```mermaid
flowchart LR
    A[组件挂载] --> B[读取 nodePositionsFromStore]
    B --> C[initialNodes 使用保存位置]
    A --> D[读取 sessionViewport]
    D --> E{有保存状态?}
    E -->|是| F[setViewport 恢复]
    E -->|否| G[fitView 自适应]
```
...
graph TB
    subgraph InteractionComponents[交互组件]
        MDP[MessageDetailPanel<br/>消息详情面板]
        NAB[NodeActionBar<br/>节点悬浮操作栏]
        NCP[NodeCreatePopover<br/>节点创建面板]
        TSQ[TextSelectionQuote<br/>文本选中引用]
        TNS[TargetNodeSelector<br/>目标节点选择器]
        TTB[TreeToolbar<br/>工具栏]
        SBB[SelectionBoundingBox<br/>框选工具]
    end
    
    subgraph Triggers[触发方式]
        T1[单击节点] --> MDP
        T2[鼠标悬浮] --> NAB
        T3[点击 Output Handle] --> NCP
        T4[选中文字] --> TSQ
        T5[底部输入框] --> TNS
        T6[顶部工具栏] --> TTB
        T7[拖拽空白处] --> SBB
...
| 组件 | 文件 | 功能 |
|-----|------|------|
| MessageDetailPanel | `MessageDetailPanel.tsx` | 右侧常驻可调节面板，显示完整消息内容，支持流式输出和多模型状态 |
| TreeToolbar | `TreeToolbar.tsx` | 顶部工具栏：模式切换（点击/框选）、聚焦、删除、自动整理布局 |
| SelectionBoundingBox | `SelectionBoundingBox.tsx` | 框选多个节点进行批量操作 |
...
#### 夜间模式适配
- 深度适配 Mantine 和 Tailwind 夜间模式
- 节点、连线、背景点阵、Minimap 均支持自动切换
- 确保在深色背景下的高对比度和可读性

#### 多模型对话支持
- 节点视图完整支持多模型并发对话
- `AssistantNode` 和 `MessageDetailPanel` 能够展示多模型生成状态
- 支持针对特定模型进行重新生成
...
flowchart TD
    A[用户输入]
    B[sessionActions.sendMessage 处理消息]
    C[AI SDK 调用 streamText]
    D[流式响应处理 onChunk]
    E[更新ChatStore 保存消息]
...
### Q: 节点视图支持哪些交互？
**A:** 
- 单击节点：打开右侧消息详情面板（可调整宽度）
- 双击节点：编辑消息内容
- 鼠标悬浮：显示底部操作栏（编辑、复制、引用、删除、重新生成）
- 顶部工具栏：切换选择模式、自动整理布局、聚焦选中节点
- 框选操作：拖拽空白区域可框选多个节点进行批量删除
...
**更新日期**: 2025-12-27
**项目版本**: 1.0.0
**节点视图功能**: 阶段一~四已完成（基础设施、核心视图、交互功能、体验优化）
